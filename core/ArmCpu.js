"use strict";
let replacedFunctions = {};
function $replace(func, ...replacements) {
...
function rotateRight32(val, bits) {
    return (val >> bits) | (val << -bits);
}
function arithmeticShiftRight32(val, bits) {
    return val >> bits;
}
function logicalShiftLeft32(val, bits) {
    return val << bits;
}
function logicalShiftRight32(val, bits) {
    return val >>> bits;
}
function checkOverflowSub(val1, val2, result) {
    return ((val1 ^ val2) & ((val1 ^ result)) & 0x80000000) != 0;
}
function checkOverflowAdd(val1, val2, result) {
    return (~(val1 ^ val2) & ((val1 ^ result)) & 0x80000000) != 0;
}
function checkBinaryMask(val, mask) {
...
// BX
function armBranchWithExchange(ins) {
    let rm = ins & 0xF;
...
function armMsrRegister(ins) {
    ...
    let byteMask = (setControl ? 0x000000FF : 0) |
...
function generateArmDataProcessing(ins) {
    ...
    let func = function (ins) {
        let regShift = $replace(0);
        let shiftType = $replace(1);
        let setFlags = $replace(2);
        let opcode = $replace(3);
        let useImmediate = $replace(4);
        let rn = (ins >> 16) & 0xF;
        let rd = (ins >> 12) & 0xF;
        let rnVal = this.r[rn];
    ...
    return $replace(func, _regShift, _shiftType, _setFlags, _opcode, _useImmediate);
}
function generateArmRegularLdrStr(ins) {
    ...
    let func = function (ins) {
        let useRegisterOffset = $replace(0);
        let p = $replace(1);
        let u = $replace(2);
        let b = $replace(3);
        let w = $replace(4);
    ...
    return $replace(func, _useRegisterOffset, _p, _u, _b, _w, _l);
}
function generateArmMiscellaneousLdrStr(ins) {
    ...
    return $replace(func, _h, _s, _l, _w, _immediateOffset, _u, _p);
}
function generateArmLdmStm(ins) {
...
function thumbLdrLiteralPool(ins) {
    let rd = (ins >> 8) & 0b111;
...
function generateThumbDataProcessing(ins) {
...
function generateThumbShiftByImmediate(ins) {
    let opcode = (ins >> 11) & 3;
    return function (ins) {
...
function thumbConditionalBranch(ins) {
    let cond = (ins >> 8) & 0xF;
...
function generateThumbAddSubCmpMovImmediate(ins) {
...
function generateThumbLdmiaStmia(ins) {
...
function thumbSpecialDataMov(ins) {
...
function thumbSpecialDataBx(ins) {
...
function thumbPush(ins) {
    let addr = this.r[13];
    addr -= popCount(ins & 0x1FF) * 4;
    addr &= 0xFFFFFFFF;
    if (bitTest(ins, 0)) { /* regs += "R0 "; */
        this.write32(addr & ~3, this.r[0]);
        addr += 4;
        this.r[13] -= 4;
    }
    if (bitTest(ins, 1)) { /* regs += "R1 "; */
        this.write32(addr & ~3, this.r[1]);
        addr += 4;
        this.r[13] -= 4;
    }
    if (bitTest(ins, 2)) { /* regs += "R2 "; */
        this.write32(addr & ~3, this.r[2]);
        addr += 4;
        this.r[13] -= 4;
    }
    if (bitTest(ins, 3)) { /* regs += "R3 "; */
        this.write32(addr & ~3, this.r[3]);
        addr += 4;
        this.r[13] -= 4;
    }
    if (bitTest(ins, 4)) { /* regs += "R4 "; */
        this.write32(addr & ~3, this.r[4]);
        addr += 4;
        this.r[13] -= 4;
    }
    if (bitTest(ins, 5)) { /* regs += "R5 "; */
        this.write32(addr & ~3, this.r[5]);
        addr += 4;
        this.r[13] -= 4;
    }
    if (bitTest(ins, 6)) { /* regs += "R6 "; */
        this.write32(addr & ~3, this.r[6]);
        addr += 4;
        this.r[13] -= 4;
    }
    if (bitTest(ins, 7)) { /* regs += "R7 "; */
        this.write32(addr & ~3, this.r[7]);
        addr += 4;
        this.r[13] -= 4;
    }
    if (bitTest(ins, 8)) {
...
function thumbPop(ins) {
    // String regs = "";
    let addr = this.r[13];
    let registerCount = popCount(ins & 0x1FF);
    this.r[13] = addr + registerCount * 4;
    if (bitTest(ins, 0)) { /* regs += "R0 "; */
        this.r[0] = this.read32(addr & ~3);
        addr += 4;
    }
    if (bitTest(ins, 1)) { /* regs += "R1 "; */
        this.r[1] = this.read32(addr & ~3);
        addr += 4;
    }
    if (bitTest(ins, 2)) { /* regs += "R2 "; */
        this.r[2] = this.read32(addr & ~3);
        addr += 4;
    }
    if (bitTest(ins, 3)) { /* regs += "R3 "; */
        this.r[3] = this.read32(addr & ~3);
        addr += 4;
    }
    if (bitTest(ins, 4)) { /* regs += "R4 "; */
        this.r[4] = this.read32(addr & ~3);
        addr += 4;
    }
    if (bitTest(ins, 5)) { /* regs += "R5 "; */
        this.r[5] = this.read32(addr & ~3);
        addr += 4;
    }
    if (bitTest(ins, 6)) { /* regs += "R6 "; */
        this.r[6] = this.read32(addr & ~3);
        addr += 4;
    }
    if (bitTest(ins, 7)) { /* regs += "R7 "; */
        this.r[7] = this.read32(addr & ~3);
        addr += 4;
    }
    if (bitTest(ins, 8)) {
        /* regs += "PC "; */
...
function resolveArmExecutor(ins) {
    ...
    else if (checkBinaryMask(ins, "000xxxxxxxxxxxxxxxxxxxx0xxxx") ||
...
function resolveThumbExecutor(ins) {
...
class ArmCpu {
    constructor(isArmV5, onStateChange, read8, write8, read16, write16, read32, write32) {
        this.executedNum = 0;
        this.preExecutionHook = null;
        this.cpsrNegative = false;
        this.cpsrZero = false;
        this.cpsrCarry = false;
        this.cpsrOverflow = false;
        this.cpsrSticky = false;
        this.cpsrIrqDisable = false;
        this.cpsrFiqDisable = false;
        this.cpsrThumbState = false;
        this.mode = 31 /* SYS */;
        this.spsrFiq = 0;
        this.spsrSvc = 0;
    ...
    generateArmExecutorTable() {
        var _a;
        let table = new Array(4096);
        for (let i = 0; i < 4096; i++) {
            let ins = ((i & 0xFF0) << 16) | ((i & 0xF) << 4);
            table[i] = (_a = resolveArmExecutor(ins)) === null || _a === void 0 ? void 0 : _a.bind(this);
        }
        return table;
    }
    generateThumbExecutorTable() {
        var _a;
        let table = new Array(1024);
        for (let i = 0; i < 1024; i++) {
            let ins = i << 6;
            table[i] = (_a = resolveThumbExecutor(ins)) === null || _a === void 0 ? void 0 : _a.bind(this);
        }
        return table;
    }
    getMode() {
    ...
    setMode(mode) {
        ...
        switch (this.mode) {
            ...
            case 31 /* SYS */:
                for (let i = 0; i < 7; i++)
                    this.rUsr[i] = this.r[8 + i];
                break;
            case 17 /* FIQ */:
                for (let i = 0; i < 7; i++)
                    this.rFiq[i] = this.r[8 + i];
                break;
            case 19 /* SVC */:
                for (let i = 0; i < 2; i++)
                    this.rSvc[i] = this.r[13 + i];
                break;
            case 23 /* ABT */:
                for (let i = 0; i < 2; i++)
                    this.rAbt[i] = this.r[13 + i];
                break;
            case 18 /* IRQ */:
                for (let i = 0; i < 2; i++)
                    this.rIrq[i] = this.r[13 + i];
                break;
            case 27 /* UND */:
                for (let i = 0; i < 2; i++)
                    this.rUnd[i] = this.r[13 + i];
                break;
        }
        switch (mode) {
            case 16 /* USR */:
            case 31 /* SYS */:
                for (let i = 5; i < 7; i++)
                    this.r[8 + i] = this.rUsr[i];
                break;
            case 17 /* FIQ */:
                for (let i = 0; i < 7; i++)
                    this.r[8 + i] = this.rFiq[i];
                break;
            case 19 /* SVC */:
                for (let i = 0; i < 2; i++)
                    this.r[13 + i] = this.rSvc[i];
                break;
            case 23 /* ABT */:
                for (let i = 0; i < 2; i++)
                    this.r[13 + i] = this.rAbt[i];
                break;
            case 18 /* IRQ */:
                for (let i = 0; i < 2; i++)
                    this.r[13 + i] = this.rIrq[i];
                break;
            case 27 /* UND */:
                for (let i = 0; i < 2; i++)
                    this.r[13 + i] = this.rUnd[i];
                break;
        }
        if (this.mode == 17 /* FIQ */)
            for (let i = 0; i < 5; i++)
    ...
    getCpsr() {
        ...
        if (this.cpsrSticky)
            val |= bit(27);
        if (this.cpsrIrqDisable)
            val |= bit(7);
        if (this.cpsrFiqDisable)
            val |= bit(6);
        if (this.cpsrThumbState)
            val |= bit(5);
        val |= this.getMode();
